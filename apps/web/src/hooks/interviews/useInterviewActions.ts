...
export function useInterviewActions() {
  ...
  // Schedule/update go through InterviewDialog, which shows `error` inline —
  // toasting those too would double-signal the same failure. Outcome/delete
  // are quick card actions with no dialog, so a toast is their only surface.
  ...
  const setOutcome = useCallback(
    (interviewId: string, outcome: InterviewOutcome) =>
      run(interviewId, () => outcomeMutation({ variables: { interviewId, outcome } }), { toastOnError: true }),
    [run, outcomeMutation],
  );

  const deleteInterview = useCallback(
    (interviewId: string) => run(interviewId, () => deleteMutation({ variables: { interviewId } }), { toastOnError: true }),
    [run, deleteMutation],
  );
